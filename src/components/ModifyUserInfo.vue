<template>
  <linyu-modal :is-open="isOpen">
    <div class="modify-user-info" @click.stop>
      ...
      <linyu-input v-model:value="avatar" label="头像URL" placeholder="请输入头像URL" height="40px" font-size="14px"/>
      <linyu-input v-model:value="name" label="用户名" placeholder="请输入用户名" height="40px" font-size="14px"
                   :readonly="true"/>
      <linyu-input v-model:value="email" label="邮箱" placeholder="请输入邮箱" height="40px" font-size="14px"
                   :readonly="true"/>
...
.modify-user-info {
  width: 500px;
  height: 350px;
  background-image: linear-gradient(130deg, rgba(var(--background-color), 0.7), rgba(var(--background-color), 0.9));
